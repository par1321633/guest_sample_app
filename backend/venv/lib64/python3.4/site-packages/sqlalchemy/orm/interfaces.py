...
class MapperProperty(_MappedAttribute, InspectionAttr, util.MemoizedSlots):
    ...
    def class_attribute(self):
        ...
        and the class to which it is mapped is ``Admin``, this sequence
        is possible::

            >>> from sqlalchemy import inspect
            >>> mapper = inspect(Admin)
            >>> addresses_property = mapper.attrs.addresses
            >>> addresses_property.class_attribute is Admin.addresses
            True
            >>> Admin.addresses.property is addresses_property
            True
...
class PropComparator(operators.ColumnOperators):
    ...
    Admin-defined subclasses of :class:`.PropComparator` may be created. The
