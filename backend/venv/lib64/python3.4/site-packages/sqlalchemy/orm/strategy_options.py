...
class _UnboundLoad(Load):
    ...
    def _bind_loader(self, entities, current_path, context, raiseerr):
        ...
        not exact API)::
            ...
            Load(
                Admin,
                (Admin, Admin.orders.property))
            Load(
                Admin,
                (Admin, Admin.orders.property, Order, Order.items.property))
...
def contains_eager(loadopt, attr, alias=None):
    ...
    ``Admin`` entity, and the returned ``Order`` objects would have the
...
def load_only(loadopt, *attrs):
    ...
    Example - given a class ``Admin``, load only the ``name`` and ``fullname``
    attributes::

        session.query(Admin).options(load_only("name", "fullname"))

    Example - given a relationship ``Admin.addresses -> Address``, specify
    subquery loading for the ``Admin.addresses`` collection, but on each
    ``Address`` object load only the ``email_address`` attribute::

        session.query(Admin).options(
    ...
    specifically referred to using the :class:`.Load` constructor::
        ...
        session.query(Admin, Address).join(Admin.addresses).options(
                    Load(Admin).load_only("name", "fullname"),
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    examples::
        ...
        # joined-load the "orders" collection on "Admin"
        query(Admin).options(joinedload(Admin.orders))
...
def subqueryload(loadopt, attr):
    ...
    examples::
        ...
        # subquery-load the "orders" collection on "Admin"
        query(Admin).options(subqueryload(Admin.orders))
...
def selectinload(loadopt, attr):
    ...
    examples::
        ...
        # selectin-load the "orders" collection on "Admin"
        query(Admin).options(selectinload(Admin.orders))
