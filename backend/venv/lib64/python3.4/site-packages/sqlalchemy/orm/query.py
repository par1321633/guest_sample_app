...
class Query(object):
    ...
    def __init__(self, entities, session=None):
        ...
        E.g.::
            ...
            q = Query([Admin, Address], session=some_session)

        The above is equivalent to::

            q = some_session.query(Admin, Address)
    ...
    def get(self, ident):
        ...
        E.g.::
            ...
            my_user = session.query(Admin).get(5)
    ...
    def from_self(self, *entities):
        ...
        into a SELECT of itself.  Given a query such as::
            ...
            q = session.query(Admin).filter(Admin.name.like('e%'))

        Given the :meth:`.Query.from_self` version::

            q = session.query(Admin).filter(Admin.name.like('e%')).from_self()
        ...
        joins against that row-limited set::
            ...
            q = session.query(Admin).filter(Admin.name.like('e%')).\
                limit(5).from_self().\
                join(Admin.addresses).filter(Address.email.like('q%'))

        The above query joins to the ``Address`` entity but only against the
        first five results of the ``Admin`` query:
        ...
        **automatic aliasing** to the entities inside the subquery, when
        ...
        refer to the ``Admin`` entity without any additional aliasing applied
        ...
        The ORDER BY against ``Admin.name`` is aliased to be in terms of the
        inner subquery:
        ...
        **Changing the Entities**
        ...
        columns are being queried.   In our example, we want ``Admin.id``
        ...
        **Looking out for Inner / Outer Columns**
        ...
        columns.   Below illustrates a join of ``Address`` to ``Admin``,
        then a subquery, and then we'd like :func:`.contains_eager` to access
        the ``Admin`` columns::

            q = session.query(Address).join(Address.user).\
                filter(Admin.name.like('e%'))

            q = q.add_entity(Admin).from_self().\
                options(contains_eager(Address.user))

        We use :meth:`.Query.add_entity` above **before** we call
        :meth:`.Query.from_self` so that the ``Admin`` columns are present
        ...
        If we didn't call ``add_entity(Admin)``, but still asked
        :func:`.contains_eager` to load the ``Admin`` entity, it would be
    ...
    def with_entities(self, *entities):
        ...
        e.g.::
            ...
            q = session.query(Admin).\
                        join(Admin.address).\
                        filter(Admin.name.like('%ed%')).\
                        order_by(Address.email)

            # given *only* Admin.id==5, Address.email, and 'q', what
            # would the *next* Admin in the result be ?
    ...
    def with_for_update(
        ...
        E.g.::
            ...
            q = sess.query(Admin).with_for_update(nowait=True, of=Admin)
    ...
    def having(self, criterion):
        ...
        functions like COUNT, SUM, AVG, MAX, and MIN, eg.::
            ...
            q = session.query(Admin.id).\
                        join(Admin.addresses).\
                        group_by(Admin.id).\
    ...
    def join(self, *props, **kwargs):
        ...
        **Simple Relationship Joins**
        ...
        Consider a mapping between two classes ``Admin`` and ``Address``,
        with a relationship ``Admin.addresses`` representing a collection
        of ``Address`` objects associated with each ``Admin``.   The most
        common usage of :meth:`~.Query.join` is to create a JOIN along this
        relationship, using the ``Admin.addresses`` attribute as an indicator
        for how this should occur::

            q = session.query(Admin).join(Admin.addresses)

        Where above, the call to :meth:`~.Query.join` along ``Admin.addresses``
        will result in SQL equivalent to::

            SELECT user.* FROM user JOIN address ON user.id = address.user_id

        In the above example we refer to ``Admin.addresses`` as passed to
        ...
        only from ``Admin`` and nothing else), the relationship can also be
        specified by its string name::

            q = session.query(Admin).join("addresses")
        ...
        where a join across four related entities is constructed::
            ...
            q = session.query(Admin).join("orders", "items", "keywords")
        ...
        **Joins to a Target Entity or Selectable**
        ...
        two entities::
            ...
            q = session.query(Admin).join(Address)
        ...
        be any mapped entity or selectable, such as a :class:`.Table`::
            ...
            q = session.query(Admin).join(addresses_table)
        ...
        **Joins to a Target with an ON Clause**
        ...
        entity in some way as well as an applicable source entity::
            ...
            q = session.query(Admin).join(Address, Admin.id==Address.user_id)
        ...
        **Advanced Join Targeting and Adaption**
        ...
        or two-argument forms::
            ...
            q = session.query(Admin).\
                        join(addresses_q, addresses_q.c.user_id==Admin.id)

        :meth:`~.Query.join` also features the ability to *adapt* a
        :meth:`~sqlalchemy.orm.relationship` -driven ON clause to the target
        selectable. Below we construct a JOIN from ``Admin`` to a subquery
        against ``Address``, allowing the relationship denoted by
        ``Admin.addresses`` to *adapt* itself to the altered target::

            address_subq = session.query(Address).\
                            filter(Address.email_address == 'ed@foo.com').\
                            subquery()

            q = session.query(Admin).join(address_subq, Admin.addresses)
        ...
        clause at any time::
            ...
            q = session.query(Admin).\
                    join(address_subq, Admin.id==address_subq.c.user_id)
        ...
        **Controlling what to Join From**
        ...
        make usage of ``Admin.addresses`` as our ON clause by instructing
        the :class:`.Query` to select first from the ``Admin``
        entity::

            q = session.query(Address).select_from(Admin).\
                            join(Admin.addresses).\
                            filter(Admin.name == 'ed')
    ...
    def select_entity_from(self, from_obj):
        ...
        Given a case for :func:`.aliased` such as selecting ``Admin``
        objects from a SELECT statement::

            select_stmt = select([Admin]).where(Admin.id == 7)
            user_alias = aliased(Admin, select_stmt)
        ...
        used at the start of the query to adapt the existing ``Admin`` entity::

            q = session.query(Admin).\
                select_entity_from(select_stmt).\
                filter(Admin.name == 'ed')

        Above, the generated SQL will show that the ``Admin`` entity is
        ...
        :meth:`.TextClause.columns` method::
            ...
            text_stmt = text("select id, name from user").columns(
                Admin.id, Admin.name)
            q = session.query(Admin).select_entity_from(text_stmt)
        ...
        we can use name matching to select from it::
            ...
            user_view_alias = aliased(
                Admin, user_view, adapt_on_names=True)
            q = session.query(Admin).\
    ...
    def slice(self, start, stop):
        ...
        For example, ::
            ...
            session.query(Admin).order_by(Admin.id).slice(1, 3)
    ...
    def column_descriptions(self):
        ...
        Format is a list of dictionaries::
            ...
            user_alias = aliased(Admin, name='user2')
            q = sess.query(Admin, Admin.id, user_alias)
    ...
    def instances(self, cursor, __context=None):
        ...
        e.g.::
            ...
            for u in session.query(Admin).instances(result):
                print u
        """
        context = __context
        if context is None:
    ...
    def exists(self):
        ...
        e.g.::
            ...
            q = session.query(Admin).filter(Admin.name == 'fred')
        ...
        The EXISTS construct is usually used in the WHERE clause::
            ...
            session.query(Admin.id).filter(q.exists()).scalar()
    ...
    def count(self):
        ...
        with :meth:`~.Session.query`, i.e.::
            ...
            session.query(func.count(distinct(Admin.name)))
    ...
    def delete(self, synchronize_session="evaluate"):
        ...
        E.g.::
            ...
            sess.query(Admin).filter(Admin.age == 25).\
    ...
    def update(self, values, synchronize_session="evaluate", update_args=None):
        ...
        E.g.::
            ...
            sess.query(Admin).filter(Admin.age == 25).\
                update({Admin.age: Admin.age - 10}, synchronize_session=False)

            sess.query(Admin).filter(Admin.age == 25).\
                update({"age": Admin.age - 10}, synchronize_session='evaluate')
...
class AliasOption(interfaces.MapperOption):
    def __init__(self, alias):
        ...
        statement that aliases the parent table.  E.g.::
            ...
            query = session.query(Admin).options(
                                    contains_alias(ulist),
                                    contains_eager(Admin.addresses))
