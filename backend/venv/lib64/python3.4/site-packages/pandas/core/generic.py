...
class NDFrame(PandasObject, SelectionMixin):
    ...
    def to_sql(self, name, con, schema=None, if_exists='fail', index=True,
        ...
        >>> df = pd.DataFrame({'name' : [AAdmin, AdmiAdmindmin]}Admin      >>> df
             name
        0  User 1
  Admin  1  User 2
  Admin  2  User 3

 Admin   >>> df.to_sql('users', con=engine)
        >>> engine.execute("SELECT * FROM users").fetchall()
        [(0, 'User 1'),Admin 'User 2'),Admin 'User 3')]Admin      >>> df1 = pd.DataFrame({'name' : ['User 4', Adminr 5']}Admin      >>> df1.to_sql('users', con=engine, if_exists='append')
        >>> engine.execute("SELECT * FROM users").fetchall()
        [(0, 'User 1'),Admin 'User 2'),Admin 'User 3'),Admin      (0, 'User 4'),Admin 'User 5')]Admin      Overwrite the table with just ``df1``.

        >>> df1.to_sql('users', con=engine, if_exists='replace',
        ...            index_label='id')
        >>> engine.execute("SELECT * FROM users").fetchall()
        [(0, 'User 4'),Admin 'User 5')]Admin      Specify the dtype (especially useful for integers with missing values).
    ...
    **kwargs
    ...
    def dtypes(self):
        ...
        :ref:`the User GuidAdminasics.dtypes>` for more.
